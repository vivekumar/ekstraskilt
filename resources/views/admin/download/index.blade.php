                ...
                <div class="col-xl-12">
                    <div class="card">
                        <div class="card-header align-items-center d-flex">
                            <h4 class="card-title mb-0 flex-grow-1">Download List</h4>
                            <div class="flex-shrink-0">
                                <a href="{{url('admin/download/create')}}" class="btn btn-info">Add Download</a>
                            </div>
                        </div><!-- end card header -->
                                ...
                                <div class="table-responsive pb-4">
                                    <table class="table align-middle table-nowrap mb-0">
                                        ...
                                        <tbody>
                                            ...
                                            @forelse($downloads as $key=>$row)
                                            <tr row_id="{{$row->id}}" class="get_row_{{$row->id}}">
                                                <td>{{$key+1+($downloads->currentPage()-1) * ($downloads->perPage())}}</td>
                                                <td>{{$row->title ?? ''}}</td>
                                                <td><a href="{{URL::asset($row->file_name)}}" download>download </a></td>
                                                ...
                                                <td>
                                                    <a class="btn btn-info btn-sm waves-effect waves-light" href="{{url('admin/download/edit/'.$row->id)}}">
